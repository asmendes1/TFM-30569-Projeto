...
def converter_params_para_python_BayesOpt(params):
    params_python = {}
    ...
    for param, config in params.items():
        ...
        if param_type == 'real':
            ...
            params_python[param] = Real(min_value, max_value, prior='uniform')
        elif param_type == 'int':
            range_info = config['range']
            min_value = range_info['min']
            max_value = range_info['max']
            step = range_info['step']
            params_python[param] = Integer(min_value, max_value)
...
if os.path.exists(dataset_storage_path):
    ...
    for dataset_filename in files_csv_only:
        ...
        for algorithm_data in HyP_learn_scheme["algorithms"]:
            ...
            params_python = converter_params_para_python_BayesOpt(algorithm_params_json)
            ...
            for train_idx, test_idx in skf_cv_outer.split(X, y):
                ...
                if performance_metric == "accuracy_score":
                    search = BayesSearchCV(estimator=pipeline, search_spaces=params_python, scoring='accuracy',
                    ...
                    search = BayesSearchCV(estimator=pipeline, search_spaces=params_python, scoring='f1',
                                           cv=skf_cv_inner, refit=True)
    ...
    # Sort the DataFrame based on the 'best_score' column
    df_ = df.sort_values(by='mean_score', ascending=False)
    print('========== FINAL RESULTS (BayesOpt) ==========')
    print(df_)
    ...
    filename = f'results_BayesOpt_{current_datetime}.csv'
